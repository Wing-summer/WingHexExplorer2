...
int main() {
    ...
    WingCStruct::parser parser("WinNTPE.h");
    ...
    auto idh = parser.read(0, "IMAGE_DOS_HEADER");
    ...
    e = parser.containsConstVar("IMAGE_DOS_SIGNATURE");
    if (!e) {
    ...
    auto structSize = parser.sizeOf("IMAGE_DOS_HEADER");
    ...
    auto ifhoff = peoff + 4;
    auto ifh = parser.read(ifhoff, "IMAGE_FILE_HEADER");
    ...
    structSize = parser.sizeOf("IMAGE_FILE_HEADER");
    ...
    auto x32sig = parser.constVarValueInt("IMAGE_FILE_MACHINE_I386");
    auto x64sig = parser.constVarValueInt("IMAGE_FILE_MACHINE_AMD64");
    ...
    if (x32sig == mcsig) {
        print("X86 PE file\n");
        e = parser.containsStruct("IMAGE_OPTIONAL_HEADER32");
        if (!e) {
            print("IMAGE_OPTIONAL_HEADER32 not found!");
            cleanUp(handle);
            return NOT_FOUND_HEADER;
        }
        auto ioh = parser.read(iohoff, "IMAGE_OPTIONAL_HEADER32");
        if (ioh.isEmpty()) {
        ...
        structSize = parser.sizeOf("IMAGE_OPTIONAL_HEADER32");
        ...
        auto ioh = parser.read(iohoff, "IMAGE_OPTIONAL_HEADER64");
        if (ioh.isEmpty()) {
        ...
        structSize = parser.sizeOf("IMAGE_OPTIONAL_HEADER64");
        ctl::metadata(iohoff, structSize, color(), color("#3e80d3"), "");
        iddoff = iohoff + structSize;
    } else {
    ...
    structSize = parser.sizeOf("IMAGE_SECTION_HEADER");
