...
static tm
time_point_to_tm(const std::chrono::time_point<std::chrono::system_clock> &tp) {
...
tm_to_time_point(const tm &_tm,
    ...
    // Adjust the time_t since epoch with the difference of the local timezone
    // to the universal timezone
    struct tm local_tm, utc_tm;
#if defined(_WIN32) || defined(_WIN64)
    // MSVC / Windows: localtime_s, gmtime_s
    localtime_s(&local_tm, &t);
    gmtime_s(&utc_tm, &t);
#else
    // POSIX: localtime_r, gmtime_r (thread-safe)
...
bool CDateTime::setDate(asUINT year, asUINT month, asUINT day) {
    ...
    if (local.tm_year != local2.tm_year || local.tm_mon != local2.tm_mon ||
...
bool CDateTime::setTime(asUINT hour, asUINT minute, asUINT second) {
    ...
    if (local.tm_hour != local2.tm_hour || local.tm_min != local2.tm_min ||
...
CDateTime::CDateTime(asUINT year, asUINT month, asUINT day, asUINT hour,
                     asUINT minute, asUINT second) {
...
asINT64 CDateTime::operator-(const CDateTime &dt) const {
    return (tp - dt.tp).count() / std::chrono::system_clock::period::den *
           std::chrono::system_clock::period::num;
}

CDateTime CDateTime::operator+(asINT64 seconds) const {
    CDateTime dt(*this);
    dt.tp += std::chrono::system_clock::duration(
        seconds * std::chrono::system_clock::period::den /
        std::chrono::system_clock::period::num);
    return dt;
}

CDateTime &CDateTime::operator+=(asINT64 seconds) {
    tp += std::chrono::system_clock::duration(
        seconds * std::chrono::system_clock::period::den /
        std::chrono::system_clock::period::num);
    return *this;
}
...
CDateTime &CDateTime::operator-=(asINT64 seconds) { return *this += -seconds; }
...
system_clock::time_point CDateTime::time_point() const { return tp; }
...
static void Construct(CDateTime *mem) { new (mem) CDateTime(); }

static void ConstructCopy(CDateTime *mem, const CDateTime &o) {
    new (mem) CDateTime(o);
}

static void ConstructSet(CDateTime *mem, asUINT year, asUINT month, asUINT day,
                         asUINT hour, asUINT minute, asUINT second) {
...
void RegisterScriptDateTime(asIScriptEngine *engine) {
    ...
    if (strstr(asGetLibraryOptions(), "AS_MAX_PORTABILITY") == 0) {
        ...
        r = engine->RegisterObjectBehaviour(
            "datetime", asBEHAVE_CONSTRUCT, "void f()",
            WRAP_OBJ_LAST(Construct), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectBehaviour(
            "datetime", asBEHAVE_CONSTRUCT, "void f(const datetime &in)",
            WRAP_OBJ_FIRST(ConstructCopy), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectBehaviour(
            "datetime", asBEHAVE_CONSTRUCT,
            "void f(uint, uint, uint, uint = 0, uint = 0, uint = 0)",
            asFUNCTION(ConstructSet_Generic), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "datetime &opAssign(const datetime &in)",
            WRAP_MFN(CDateTime, operator=), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "uint get_year() const property",
            WRAP_MFN(CDateTime, getYear), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "uint get_month() const property",
            WRAP_MFN(CDateTime, getMonth), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "uint get_day() const property",
            WRAP_MFN(CDateTime, getDay), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "uint get_hour() const property",
            WRAP_MFN(CDateTime, getHour), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "uint get_minute() const property",
            WRAP_MFN(CDateTime, getMinute), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "uint get_second() const property",
            WRAP_MFN(CDateTime, getSecond), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "uint get_weekDay() const property",
            WRAP_MFN(CDateTime, getWeekDay), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "bool setDate(uint year, uint month, uint day)",
            WRAP_MFN(CDateTime, setDate), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "bool setTime(uint hour, uint minute, uint second)",
            WRAP_MFN(CDateTime, setTime), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "int64 opSub(const datetime &in) const",
            WRAP_MFN_PR(CDateTime, operator-, (const CDateTime &other) const,
                        asINT64),
            asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
        ...
        r = engine->RegisterObjectMethod(
            "datetime", "datetime opAdd_r(int64 seconds) const",
            WRAP_OBJ_LAST_PR(operator+,
                             (asINT64 seconds, const CDateTime &other),
                             CDateTime),
            asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
        ...
        r = engine->RegisterObjectMethod(
            "datetime", "datetime opSub(int64 seconds) const",
            WRAP_MFN_PR(CDateTime, operator-, (asINT64) const, CDateTime),
            asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterObjectMethod(
            "datetime", "datetime opSub_r(int64 seconds) const",
        ...
        r = engine->RegisterObjectMethod("datetime",
                                         ...
                                         WRAP_OBJ_FIRST(opCmp), asCALL_GENERIC);
        assert(r >= 0);
    }
}
