...
struct SContextInfo {
    asUINT sleepUntil;
    QVector<asIScriptContext *> coRoutines;
...
static void ScriptSleep(asUINT milliSeconds) {
    ...
    if (ctx) {
        ...
        CContextMgr *ctxMgr = reinterpret_cast<CContextMgr *>(
        ...
        if (ctxMgr) {
            ...
            // Tell the context manager when the context is to continue
            // execution
...
static void ScriptYield() {
    ...
    if (ctx) {
        ...
        CContextMgr *ctxMgr = reinterpret_cast<CContextMgr *>(
            ctx->GetUserData(AsUserDataType::UserData_ContextMgr));
        if (ctxMgr) {
            // Let the context manager know that it should run the next
            // co-routine
...
void ScriptCreateCoRoutine(asIScriptFunction *func, CScriptDictionary *arg) {
    ...
    if (ctx) {
        ...
        if (ctxMgr) {
            ...
            // The context manager will call Execute() on the context when it is
            // time
        }
    }
}
    ...
    for (n = 0; n < m_threads.size(); n++) {
        if (m_threads[n]) {
            for (asUINT c = 0; c < m_threads[n]->coRoutines.size(); c++) {
                ...
                if (ctx) {
                    // Return the context to the engine (and possible context
                    // pool configured in it)
...
qsizetype CContextMgr::ExecuteScripts() {
    // TODO: Should have an optional time out for this function. If not all
    // scripts executed before the
    //       time out, the next time the function is called the loop should
    //       continue where it left off.

    // TODO: There should be a time out per thread as well. If a thread executes
    // for too
    //       long, it should be aborted. A group of co-routines count as a
    //       single thread.

    // Check if the system time is higher than the time set for the contexts
    asUINT time = m_getTimeFunc ? m_getTimeFunc() : asUINT(-1);
    for (m_currentThread = 0; m_currentThread < m_threads.size();
         m_currentThread++) {
        ...
        if (thread->sleepUntil < time) {
            ...
            asIScriptEngine *engine =
                thread->coRoutines[currentCoRoutine]->GetEngine();
            ...
            if (r != asEXECUTION_SUSPENDED) {
                // The context has terminated execution (for one reason or
                // other) Unless the application has requested to keep the
                // context we'll return it to the pool now
                if (thread->keepCtxAfterExecution !=
                    thread->coRoutines[currentCoRoutine])
                    engine->ReturnContext(thread->coRoutines[currentCoRoutine]);
                thread->coRoutines[currentCoRoutine] = nullptr;
            ...
            // TODO: If more objects are created per execution than destroyed on
            // average
            //       then it may be necessary to run more iterations of the
            //       detection of cyclic references. At the startup of an
            //       application there is usually a lot of objects created that
            //       will live on through out the application so the average
            //       number of objects created per execution will be higher than
            //       the number of destroyed objects in the beginning, but
            //       afterwards it usually levels out to be more or less equal.
...
void CContextMgr::AbortAll() {
    ...
    for (asUINT n = 0; n < m_threads.size(); n++) {
        ...
        m_threads[n]->coRoutines.clear();

        m_freeThreads.append(m_threads[n]);
    }
...
asIScriptContext *CContextMgr::AddContext(asIScriptEngine *engine,
                                          asIScriptFunction *func,
                                          bool keepCtxAfterExec) {
...
asIScriptContext *CContextMgr::AddContextForCoRoutine(asIScriptContext *currCtx,
                                                      asIScriptFunction *func) {
...
void CContextMgr::SetSleeping(asIScriptContext *ctx, asUINT milliSeconds) {
    ...
    for (asUINT n = 0; n < m_threads.size(); n++) {
        if (m_threads[n]->coRoutines[m_threads[n]->currentCoRoutine] == ctx) {
            m_threads[n]->sleepUntil =
                (m_getTimeFunc ? m_getTimeFunc() : 0) + milliSeconds;
        }
    }
}
