// copy from angelscript/addon and add copying context
...
public:
    ...
    // Set the function that the manager will use to obtain the time in
    // milliseconds
    ...
    asIScriptContext *AddContext(asIScriptEngine *engine,
    ...
    asIScriptContext *AddContextForCoRoutine(asIScriptContext *currCtx,
                                             asIScriptFunction *func);

    // Execute each script that is not currently sleeping. The function returns
    // after each script has been executed once. The application should call
    // this function for each iteration of the message pump, or game loop, or
    // whatever. Returns the number of scripts still in execution.
    qsizetype ExecuteScripts();
...
protected:
    QVector<SContextInfo *> m_threads;
    QVector<SContextInfo *> m_freeThreads;
