...
struct OptionSetAs : public OptionSet<OptionsAs> {
	OptionSetAs() {
		DefineProperty("styling.within.preprocessor", &OptionsAs::stylingWithinPreprocessor,
			"For C++ code, determines whether all preprocessor code is styled in the "
			"preprocessor style (0, the default) or only from the initial # to the end "
			"of the command word(1).");

		DefineProperty("lexer.as.allow.dollars", &OptionsAs::identifiersAllowDollars,
			"Set to 0 to disallow the '$' character in identifiers with the cpp lexer.");

		DefineProperty("lexer.as.track.preprocessor", &OptionsAs::trackPreprocessor,
			"Set to 1 to interpret #if/#else/#endif to grey out code that is not active.");

		DefineProperty("lexer.as.update.preprocessor", &OptionsAs::updatePreprocessor,
			"Set to 1 to update preprocessor definitions when #define found.");

		DefineProperty("lexer.as.verbatim.strings.allow.escapes", &OptionsAs::verbatimStringsAllowEscapes,
			"Set to 1 to allow verbatim strings to contain escape sequences.");

		DefineProperty("lexer.as.triplequoted.strings", &OptionsAs::triplequotedStrings,
			"Set to 1 to enable highlighting of triple-quoted strings.");

		DefineProperty("lexer.as.hashquoted.strings", &OptionsAs::hashquotedStrings,
			"Set to 1 to enable highlighting of hash-quoted strings.");

		DefineProperty("lexer.as.backquoted.strings", &OptionsAs::backQuotedStrings,
			"Set to 1 to enable highlighting of back-quoted raw strings .");

		DefineProperty("lexer.as.escape.sequence", &OptionsAs::escapeSequence,
			"Set to 1 to enable highlighting of escape sequences in strings");

		DefineProperty("fold", &OptionsAs::fold);

		DefineProperty("fold.as.syntax.based", &OptionsAs::foldSyntaxBased,
			"Set this property to 0 to disable syntax based folding.");

		DefineProperty("fold.comment", &OptionsAs::foldComment,
			"This option enables folding multi-line comments and explicit fold points when using the C++ lexer. "
			"Explicit fold points allows adding extra folding by placing a //{ comment at the start and a //} "
			"at the end of a section that should fold.");

		DefineProperty("fold.as.comment.multiline", &OptionsAs::foldCommentMultiline,
			"Set this property to 0 to disable folding multi-line comments when fold.comment=1.");

		DefineProperty("fold.as.comment.explicit", &OptionsAs::foldCommentExplicit,
			"Set this property to 0 to disable folding explicit fold points when fold.comment=1.");

		DefineProperty("fold.as.explicit.start", &OptionsAs::foldExplicitStart,
			"The string to use for explicit fold start points, replacing the standard //{.");

		DefineProperty("fold.as.explicit.end", &OptionsAs::foldExplicitEnd,
			"The string to use for explicit fold end points, replacing the standard //}.");

		DefineProperty("fold.as.explicit.anywhere", &OptionsAs::foldExplicitAnywhere,
			"Set this property to 1 to enable explicit fold points anywhere, not just in line comments.");

		DefineProperty("fold.as.preprocessor.at.else", &OptionsAs::foldPreprocessorAtElse,
			"This option enables folding on a preprocessor #else or #endif line of an #if statement.");

		DefineProperty("fold.preprocessor", &OptionsAs::foldPreprocessor,
			"This option enables folding preprocessor directives when using the C++ lexer. "
			"Includes C#'s explicit #region and #endregion folding directives.");

		DefineProperty("fold.compact", &OptionsAs::foldCompact);

		DefineProperty("fold.at.else", &OptionsAs::foldAtElse,
			"This option enables C++ folding on a \"} else {\" line of an if statement.");

		DefineWordListSets(asWordLists);
	}
};
...
class LexerAs : public ILexerWithMetaData {
	bool caseSensitive;
...
public:
	explicit LexerAs(bool caseSensitive_) :
	...
	static ILexer *LexerFactoryAs() {
		return new LexerAs(true);
	}
	static ILexer *LexerFactoryAsInsensitive() {
		return new LexerAs(false);
	}
...
Sci_Position SCI_METHOD LexerAs::PropertySet(const char *key, const char *val) {
	if (osCPP.PropertySet(&options, key, val)) {
		if (strcmp(key, "lexer.as.allow.dollars") == 0) {
...
Sci_Position SCI_METHOD LexerAs::WordListSet(int n, const char *wl) {
...
void SCI_METHOD LexerAs::Lex(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument *pAccess) {
...
void SCI_METHOD LexerAs::Fold(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument *pAccess) {
...
void LexerAs::EvaluateTokens(std::vector<std::string> &tokens, const SymbolTable &preprocessorDefinitions) {
...
std::vector<std::string> LexerAs::Tokenize(const std::string &expr) const {
	// Break into tokens
...
bool LexerAs::EvaluateExpression(const std::string &expr, const SymbolTable &preprocessorDefinitions) {
...
LexerModule lmAs(SCLEX_AS, LexerAs::LexerFactoryAs, "as", asWordLists);
LexerModule lmAsNoCase(SCLEX_AS, LexerAs::LexerFactoryAsInsensitive, "asnocase", asWordLists);
