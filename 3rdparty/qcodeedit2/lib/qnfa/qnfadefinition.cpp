...
public:
    QNFANotifier(const QString &line) { m_formats.fill(0, line.length()); }

    QNFANotifier(const QDocumentLine &line) : m_line(line) {
    ...
    const QVector<int> &formats() const { return m_formats; }

    const QVector<QParenthesis> &parentheses() const { return m_parens; }

    virtual void matched(int pos, int len, int action) {
        // qDebug("action 0x%x on [%s]", action,
        // qPrintable(m_line.text().mid(pos, len)));
        ...
        if (!(action & QNFAAction::Content) &&
            (action & (QNFAAction::ParenOpen | QNFAAction::ParenClose |
                       QNFAAction::Indent | QNFAAction::Fold))) {
            // qDebug("matched paren : %s", qPrintable(m_line.text().mid(pos,
            // len)));

            QParenthesis par((action & QNFAAction::ParenMask) >> 8, 0, pos,
                             len);
...
QHash<QString, QNFADefinition::EmbedRequestList>
    QNFADefinition::m_pendingEmbeds;

void QNFADefinition::load(const QString &file, QLanguageFactory::LangData *d,
                          QFormatScheme *s) {
    QFile f(file);
...
void QNFADefinition::load(QFile *f, QLanguageFactory::LangData *d,
                          QFormatScheme *s) {
    QDomDocument doc;
    doc.setContent(f);
...
void QNFADefinition::load(const QDomDocument &doc,
                          QLanguageFactory::LangData *d, QFormatScheme *s) {
...
QNFADefinition::QNFADefinition() : m_indentFold(false), m_root(0) {}

QNFADefinition::~QNFADefinition() { delete m_root; }

QString QNFADefinition::language() const { return m_language; }

QStringList QNFADefinition::extensions() const { return m_extensions; }
...
int QNFADefinition::tokenize(QDocument *d, int line, int count) {
    ...
    while ((n < count) || diffCxt) {
        ...
        // get last context nest to check for noteworthy modifications needing
        // further work
        ...
        if (prev.isValid()) {
            ...
            /*
            qDebug("reset : popping down by %i contexts",
            line.matchContext()->parents.count());

            while ( line.matchContext()->parents.count() )
                    line.matchContext()->context =
            line.matchContext()->parents.pop();
            */
        }
        ...
        // update cont state, i.e. whether or not highlighting info of next
        // block shall be updated
...
QString QNFADefinition::defaultLineMark() const { return m_defaultMark; }
                ...
                if (parens.count()) {
                    if (::match(par, parens.top())) {
                        ...
                        do {
                            ...
                            if ((tp.role & QParenthesis::Open) &&
                                (tp.role & QParenthesis::Close)) {
                                if (par.id < tp.id) {
                                    parens.pop();
                                } else {
                                    break;
...
QString QNFADefinition::indent(const QDocumentCursor &c) {
    ...
    // qDebug("line %i, column %i : %s", b.lineNumber(), c.columnNumber(),
    // qPrintable(s));
...
bool QNFADefinition::unindent(const QDocumentCursor &c, const QString &ktxt) {
    ...
    return ((p.role & QParenthesis::Close) && (p.role & QParenthesis::Indent) &&
...
int QNFADefinition::findBlockEnd(QDocument *d, int line, bool *open) {
    ...
    if (vp.count()) {
        ...
        if ((m.type == PMatch::Match) ||
            ((m.type == PMatch::Mismatch) && (m.line[0] == line) &&
             (m.column[0] == p.offset))) {
            if (open) {
                int flags = blockFlags(d, m.line[1], 0);

                *open =
                    QCE_FOLD_OPEN_COUNT(flags) | (m.type == PMatch::Mismatch);
            }
...
int QNFADefinition::blockFlags(QDocument *d, int line, int depth) const {
    ...
    // qDebug("%i : ret : %i (%i, %i)", line, ret, ret & 0xffff0000,
    // short(((short)0) | (ret & 0xffff)));
    ...
    while (it != m_pendingEmbeds.end()) {
        foreach (const EmbedRequest &request, *it) {
            if (request.target == src) {
                // qDebug("sharing one embed request between 0x%x and 0x%x at %i
                // + %i", src, dest, offset, request.index);
