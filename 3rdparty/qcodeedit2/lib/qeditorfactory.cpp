...
QEditorFactory::QEditorFactory(QSettingsServer *s)
    ...
    m_defaultScheme =
        new QFormatScheme(QCE::fetchDataFile("formats.qxf"), this);
    ...
    if (value("version").toInt() != 3) {
        ...
        beginGroup("default");
        setValue("struct", QString::number(QCodeEdit::West) + "{" +
...
QCodeEdit *QEditorFactory::editor(const QString &f,
    ...
    connect(e->editor(), SIGNAL(loaded(QEditor *, QString)), this,
            SLOT(loaded(QEditor *, QString)));

    connect(e->editor(), SIGNAL(saved(QEditor *, QString)), this,
...
/*!
        ...
        \param layout panel layout to use
*/
QCodeEdit *QEditorFactory::editor(const QString &f, QLanguageDefinition *l,
                                  QFormatScheme *s, QCodeCompletionEngine *en,
                                  const QString &layout) const {
    QCodeEdit *e = new QCodeEdit(layout.isEmpty() ? defaultLayout() : layout);
